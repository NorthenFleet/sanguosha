...
def test_play_phase_continue():
    """测试无中生有被无懈可击响应后出牌阶段继续"""
    print("=== 测试无中生有被无懈可击响应后出牌阶段继续 ===")
    ...
    # 给曹操多张手牌，包括无中生有
    wuzhong_card = Card("无中生有", CardType.TRICK, "♥", 7)
    ...
    if wuzhong_index is not None:
        used_card = player.play_card(wuzhong_index)
        print(f"{player.character.name} 使用了 无中生有")
        ...
        if response_result == "continue_play_phase":
            ...
            print(f"无中生有生效，{player.character.name} 摸了两张牌。")
